...
public class ReactorLeverInteractable : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (mission.FuelPercent.Value < 100)
        {
            fpc.SetInteractionText("[Yetersiz Enerji]");
            return;
        }

        if (mission.IsLeverPulled(leverId))
        {
            fpc.SetInteractionText("[Senkronizasyon Bekleniyor]");
            return;
        }

        fpc.SetInteractionText("[F] Sarteri Indir");
        if (Input.GetKeyDown(KeyCode.F))
            mission.PullLeverRpc(leverId);
    }
